...
class SignUpViewController: BaseClass {
                ...
                fbRequest.start{(urlResponse, requestResult) in
                    ...
                    case .success(let graphResponse):
                        ...
                        if let responseDictionary: NSDictionary = graphResponse.dictionaryValue as NSDictionary? {
                            ...
                            //                                Progress.instance.displayAlert(userMessage: Validation.call.acceptTermsPrivacy)
                            //                            } else {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        /*if segue.identifier == segueId.createProfile.rawValue {
         let obj = segue.destination as! CreateProfileVC
...
extension SignUpViewController  {
    ...
    func validations()  {
        if textFldEmailAddress.text?.trimmingCharacters(in: .whitespacesAndNewlines).count == 0 {
            ...
            Progress.instance.displayAlert(userMessage: Validation.call.passwordLimit)
        } else if textFldConfirmPassword.text?.trimmingCharacters(in: .whitespacesAndNewlines).count == 0 {
            Progress.instance.displayAlert(userMessage: Validation.call.enterConfirmPassword)
        }  else if textFldPassword.text != textFldConfirmPassword.text {
            Progress.instance.displayAlert(userMessage: Validation.call.passwordNotMatch)
        ...
        /* textFldEmailAddress.text = "ranju@gmail.com"
         textFldPassword.text = "Qwerty@1234"
    ...
    func signUpAPI(strEmail:String, strPassword:String, strSocialId: String, strRegistrationType:String, strFirstName: String,
        ...
        APIManager.sharedInstance.signUpAPI(email:strEmail, password:strPassword, token: token, registerType: strRegistrationType, socialId: strSocialId) { (response) in
