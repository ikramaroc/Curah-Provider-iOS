...
class HistoryListVC: BaseClass {
    ...
    override func viewDidLoad() {
        ...
        self.title = "History"
        tblVwHistory.rowHeight = UITableViewAutomaticDimension
        tblVwHistory.estimatedRowHeight = 100
        tblVwHistory.sectionHeaderHeight = 50
    ...
    func filerHistoryData(){
        let sortedByDate = self.modelBase?.historyList?.sorted { (History1, History2) -> Bool in
            return  self.convertToLocalDateFormatOnlyTimeToDate(utcDate:History1.date!).compare(self.convertToLocalDateFormatOnlyTimeToDate(utcDate: History2.date!)) == .orderedDescending
        }
