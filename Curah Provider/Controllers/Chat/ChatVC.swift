...
class ChatVC: UIViewController {
    ...
    func fetchProfileInfo(driverImgUrl:String?,otherUserName:String?){
        /*  if let imageUrl = (userInfo?.user?.image?.url){
         userImgVw.kf.setImage(with: URL(string: (Apis.kBaseURL +  imageUrl)), placeholder:#imageLiteral(resourceName: "user"))
         }
         lblUserName.text = userInfo?.user?.username
         if let imageUrl = driverImgUrl{
         driverImgVw.kf.setImage(with: URL(string: (Apis.kBaseURL +  imageUrl)), placeholder:#imageLiteral(resourceName: "driver"))
         }
         if let name = otherUserName{
         lblOtherUserName.text = name
         }*/
    }
    ...
    func scrollToBottom(){
        ...
        if (messageList?.conversations?.count) != nil && (messageList?.conversations?.count)! > 0
        {
