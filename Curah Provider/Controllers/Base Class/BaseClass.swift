...
class BaseClass: UIViewController {
    ...
    func underLineTextFldBottomLine(mainView:UIView, borderColor:UIColor) {
        for subVw in mainView.subviews {
            if subVw.isKind(of: UITextField.self) {
                let textFld = subVw as! UITextField
                textFld.setBottomBorder(color: borderColor)
            }
        }
    }
    ...
    func changeNavigationBarColor() {
        self.navigationController?.navigationBar.setBackgroundImage(#imageLiteral(resourceName: "navigationImage"), for: .default)
        self.navigationController?.navigationBar.shadowImage = #imageLiteral(resourceName: "navigationImage")
        self.navigationController?.navigationBar.isTranslucent = true
    }
    ...
    func conversion12hrFormatTo24hrFormat(dateStr:String) -> String{
    ...
    func conversion24hrFormatTo12hrFormat(dateStr:String) -> String{
        if dateStr.count == 0 {
            return ""
        } else {
            let dateFormatter = DateFormatter()
    ...
    func conversion24hrFormatTo12hrFormatWithSec(dateStr:String) -> String{
        if dateStr.count == 0 {
            return ""
        } else {
            let dateFormatter = DateFormatter()
    ...
    func convertToLocalOnlyDate(utcDate:Date) -> String
    {
        // create dateFormatter with UTC time format
        let dateFormatter = DateFormatter()
